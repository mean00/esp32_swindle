...
pub fn build() -> Result<EspIdfBuildOutput> {
    ...
    // Optional compiler-launcher support (sccache/ccache): when
    // ESP_IDF_SYS_C_COMPILER_LAUNCHER is set, route every ESP-IDF C/C++ compile
    // through the launcher. build_mini.sh sets this to `sccache` when available,
    // which caches the ~1200-object ESP-IDF C build in ~/.cache/sccache (it
    // survives the `rm -Rf target .embuild` clean). No-op when unset.
