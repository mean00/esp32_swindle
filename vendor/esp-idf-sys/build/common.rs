...
impl EspIdfVersion {
    ...
    /// or of the form `esp_idf_version_patch_at_least_X_Y_Z` / `esp_idf_version_patch_at_most_X_Y_Z`.
    ///
    /// X.Y.Z denotes a released ESP-IDF version which is smaller or equal to the ESP-IDF
    /// verson against which we are compiling.
    ///
    /// Thus if we are compiling e.g. against ESP-IDF 5.2.3, the iterator will yield:
    ...
    /// - ... and so on, up to `esp_idf_version_at_least_5_2_3`
    ///
...
pub fn setup_clang_env(path: Option<&Path>) -> Result<()> {
    // Prefer the ESP-IDF esp-clang libclang (via the `~/.espup/esp-clang`
    // symlink) whenever it is available, even when a `clang` binary was
    // discovered on the toolchain `$PATH`. This is required for correct bindgen
    // output: the ESP fork of clang enables the ESP-IDF `__riscv_*` feature
    // macros for `-target riscv32`, so bindgen sees the complete newlib struct
    // definitions; a generic system clang (e.g. clang 22) instead emits opaque
    // placeholder types (`_address: u8`, size 1) for `timeval`, `itimerval`,
    // `_reent`, ... which fail the `checks::libc` type assertions in this crate
    // on any cold (non-sccache) build. The discovered clang's own libclang is
    // only used as a fallback when the espup symlink is not usable.
    ...
    if policy != "ignore" {
        ...
        if let Some(espup_clang_path) = espup_clang_path {
